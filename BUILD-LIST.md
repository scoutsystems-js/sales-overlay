...
**⚠⚠ FILED ONLY. DO NOT BUILD ANY OF IT.** This is Justin's ruling recorded verbatim in shape, not a design — and the first bullet below says why nobody can start it yet.

| item | the ruling | what it actually needs |
|---|---|---|
| **Admin view rebuild — companies** | The admin view is reorganised around **COMPANIES** rather than a flat user list. ⚠ Each company gets its own **BLOCK**, ordered top-down: **Director > Manager > Users**. ⚠ The company **NAME is editable at the top centre of its block**, and that name then appears at the **top centre of the TEAM view** for that company's manager. ⚠ **DEACTIVATE and DELETE for a whole company and all its users.** ⚠ **THREE TABS**: Companies, Single Users, All Users. ⚠ An admin can go to **Team view and use a dropdown to cycle through every company** and see their sales data. | ⚠⚠ **SCOUT HAS NO COMPANY CONCEPT TODAY — IT HAS TEAMS.** Whoever picks this up must FIRST establish whether a "company" *is* a team, or a **new tier above** teams. **That answer changes everything downstream** — the schema, `resolveTeam`, every team-scoped query, and the meaning of `managed_by`. ⚠ **DEPENDS ON the filed role-hierarchy item** (Executive / Director / Manager / Closer): **Director is referenced here and does not exist yet.** ⚠ **Deleting a company deletes many users at once**, so the tombstone-vs-remove decision that shipped for single users (`b535fb5`) must apply consistently at company scale — a company delete that hard-deletes N users would destroy N people's call history and rewrite every historical figure they contributed to. |
