...
### ⚠⚠⚠ URGENT — DUAL-SOURCE INGESTION IS DOUBLE-COUNTING EVERY CALL (found 2026-08-24)

**On the account with both Fathom and Zoom connected, EOD shows 10 calls for 6 real meetings.** Each meeting is ingested twice, ~1 minute apart, proven by near-identical durations (3000/2997, 3120/3088).

**The ONE-ACTIVE-SOURCE-PER-USER ruling was filed and never built**, on the grounds that Fathom's bot creates no Zoom cloud recording so nothing would duplicate. **That reprieve has expired** — cloud recording is on and everything arrives twice.

⚠ **The existing dedupe cannot catch it:** `UNIQUE (user_id, fathom_call_id)` holds each provider's own id, so two providers never collide. ⚠ **And `meeting_id` cannot pair them** — a Personal Meeting Room reuses one id for every meeting. Pairing needs **start time within ~60s + near-identical duration**.

**This inflates every per-call figure on that account.** Not fixed this block (EOD was the scope); it is the next thing worth doing.
