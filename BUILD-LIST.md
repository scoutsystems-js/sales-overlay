...
|---|---|---|
| **⚠⚠ The wordmark font cannot draw the wordmark — THREE OPTIONS NOW RENDERED, awaiting Justin's pick** | `archivo-expanded-700.woff2` maps SPACE and `"A"` only. **"SCOUT SYSTEMS" has no `A`, so every letter on BOTH the login lockup and the welcome title has rendered in the SYSTEM FALLBACK since 2026-08-18** | **A ruling, not a bug fix — every repair changes a wordmark Justin has already approved.** Rendered side by side at the same size, one image, 2026-08-20. **A** real Archivo (k 12.146, ~26% wider than today, the appearance the code always intended; needs `--cap-em` 0.7197→0.687) · **B** Saira `wdth 125` (k 10.321, one fewer file, axis already loading) · **C** the fallback (k 9.632, approved but **OS-dependent**). All three fit both surfaces; **A's welcome margin is 1.2%, measured.** ⚠ Whatever is chosen is **INTERIM** — the new SVG logo lockup replaces every text wordmark. **My recommendation: B.** Guarded by `test/font-coverage.test.js` |
...
| **Login page + post-login animation — bring in line with the site** | Neither matches the swept product yet | **INVESTIGATED 2026-08-20; what remains is three rulings, not work.** ✅ **Saira confirmed rendering on both**, control proven to move first, `document.fonts.check` not used. ✅ Animation checked against the site: kicker is Saira 600 and `--accent` matches, and **no wrap at any frame of the settle including the widest (24.5px tracking)**. ➡ Open: **(1)** the wordmark font ruling above, which hits BOTH surfaces · **(2)** login body 300 vs 450 · **(3)** the animation's motion vocabulary is deliberately unlike the site's (durations 420–760ms vs the site's 0.15s, expo + 1.8 overshoot easings vs `cubic-bezier(0.4,0,0.2,1)`) — **appropriate for a ceremony, so recommended UNCHANGED**, but recorded so it is a choice. ⚠ Also: the three surfaces use three different geometries — concentric arcs (login), a radar dial (welcome), a node mesh (dashboard) — and only the first is the brand mark |
...
| **Wordmark → Saira wide (`wdth 125`)** — now **option B** of the wordmark ruling | Replace Archivo Expanded on the lockup so it echoes the new logo | ⚠⚠ **PREMISE WITHDRAWN (2026-08-20), AS INSTRUCTED.** This item argued *"Archivo is genuinely EXPANDED and Saira at `wdth 100` visibly shrinks the lockup."* **Both halves are void: that comparison was against the SYSTEM FALLBACK — nobody had ever seen Archivo render on this page — and it compared Saira at the WRONG WIDTH.** Measured properly at `wdth 125`: Saira **k = 10.321** (confirming this item's own figure, independently), against the real Archivo's **12.146** and the fallback's **9.632**. So Saira wide is **wider than what ships today**, not narrower. ⚠ Still needs the lockup guard's weight/tracking/k pairing updated. **FILED, NOT BUILT — now one of three rendered options in front of Justin** |
