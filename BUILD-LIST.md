...
|---|---|---|
| **⚠⚠ The wordmark font cannot draw the wordmark** | `archivo-expanded-700.woff2` maps SPACE and `"A"` only. **"SCOUT SYSTEMS" has no `A`, so every letter on BOTH the login lockup and the welcome title has rendered in the SYSTEM FALLBACK since 2026-08-18** | **A ruling, not a bug fix — all three repairs change a wordmark Justin has already approved.** **A** ship a real Archivo subset (k = 12.226 already derived for it, geometry would finally be true) · **B** the filed Saira-wide item below (k = 10.321, one fewer file, axis already loading) · **C** accept the fallback, delete the dead file and re-derive k — ⚠ but that leaves the wordmark a **different typeface on every OS**, the thing self-hosting was adopted to prevent. Measured 3 ways + looked at; guarded by `test/font-coverage.test.js` |
| **Login body weight: 300 or 450?** | Login runs 300, the site runs 450. The rule forcing 300 existed for a limitation that no longer applies | **Recommend 450.** Measured: +30.6% ink mass, advance unchanged (+0.4px), so it reads as weight not width and nothing can reflow. 300 is more elegant in isolation; 450 is what every other surface uses and is more legible on the `--muted` greys this page keeps. Side-by-side capture in the findings |
...
| **Wordmark → Saira wide (`wdth 125`)** | Replace Archivo Expanded on the lockup so it echoes the new logo | ⚠ Needs **k re-derived to 10.321** and the lockup guard's weight/tracking/k pairing updated. ⚠⚠ **ITS ORIGINAL PREMISE IS WITHDRAWN (2026-08-20):** this item argued *"Archivo is genuinely EXPANDED and Saira at `wdth 100` visibly shrinks the lockup"* — **that comparison was against the SYSTEM FALLBACK. Nobody has ever seen Archivo render on this page** (see the font ruling above). Saira covers the wordmark's glyphs, so coverage is not a blocker either way. **FILED, NOT BUILT** |
