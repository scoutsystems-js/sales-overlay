...
| item | one line | depends on |
|---|---|---|
| **Team objection drilldown — step 3, the coaching summary** | The one part that GENERATES rather than displays: what the team should do about the objections shown above | **Deliberately excluded from the steps-1+2 block, not missed.** Steps 1+2 are a view over stored fields with zero model calls; this is a Claude call and belongs in its own block with its own cost and cache decisions. ⚠ **Justin's ruling, recorded now so it is not re-litigated: NAME THE CLOSERS, at any team size** — *"John is struggling with the spouse objection… James is struggling with timing."* NOT "closers are struggling with…". The data it needs is already on the wire: `grid` carries per-closer × per-category counts and rates, and `closers` carries the names. ⚠ Watch the ~34s precedent from the Objections synthesis — a cache-miss model call on this lane will be slow and needs an honest waiting state, not a spinner that reads as broken |
