...
### Call Breakdown and Team Call Review — approved September 14, 2026

The individual Call Breakdown begins with a compact identity bar: prospect, closer, date, duration, call type, score, outcome and Fathom link. The first content is the evidence-backed deal diagnosis and move. **Follow-up Email remains a first-screen action and is never removed**: it keeps the existing generated draft and copy control before detailed moments and section grades. Team → **Call Review** sits directly below Coaching in the Team menu. It is a manager-and-owner queue for the selected real team and date range, with closer, outcome, section and ordering filters; every row opens the existing owner-safe individual call review. The queue reads stored call and grade data only and does not introduce another coaching or model pass.
...
### My Team — implemented visual scope

My Team (`team-members`) uses the implemented Observatory ground, contours, glass panels and selected navigation. Preserve the roster scope and permissions, active/inactive roles, add/move/deactivate/email/reset/delete handlers, team/date controls and the scoped Scheduled Appointments link. No auth, calendar or data change. Implemented; final deployment verification is recorded in `~/Desktop/scan-reports/observatory-2026-09-13/visor-members/release-status.md`.

### Background control scope

The eight redesigned views — Overview, Team Performance, Team Coaching, Calls, EOD Report, Daily Digest, Team Objections and My Team — no longer show a header background switch or an Account Display checkbox. Older views keep the legacy header control until redesigned. The stored `scout_bg_v1=off` preference, first-paint `data-bg="off"` application, background-off styling and reduced-motion behavior remain unchanged; removing the controls preserves saved background behavior.
...
Primary landing CTAs use the approved **physical Visor control**: a raised dark panel with a fine light edge and one small glowing green status lamp. Secondary actions are dark outlined controls. The bright flat green marketing button is retired from the landing page. This is a landing-only treatment; product dashboard action controls keep their established rule. The landing proof set uses three scrubbed real product views: **Rep Coaching Review**, **Manager Coaching**, and **Call Review**. Every public image must be scrubbed at the file level before release; never publish a prospect name, call wording, customer name, or team detail as marketing art.
