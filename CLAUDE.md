...
- **BUILD PLAN (ordered, ratified 2026-07-22 close-out):**
  1. **Welcome emails — SCOPE-UPGRADED to invite flow (product ruling), REBUILT 2026-07-23, LOCAL COMMITS, awaiting approval to push.** **Transport = Resend HTTPS API (2026-07-23): Railway blocks outbound SMTP (25/465/587/2525) on this plan tier — confirmed root cause of the 2-min console hangs (nodemailer's default 2-min connectionTimeout awaited in-request). nodemailer REMOVED; native fetch to api.resend.com with a HARD 10s AbortController bound; env = RESEND_API_KEY only (WELCOME_SMTP_* retired); sender constant `Justin <justin@scoutsystems.io>` (domain verified in Resend). Never put SMTP back on Railway.** The email no longer carries a temp password: creation mints a one-time Supabase **recovery-type action link** (`generateLink` — invite-type rejects existing users; mint REJECTS links whose redirect_to was silently swapped for the Site URL, i.e. the allowlist isn't configured yet) → email (Option D SMTP, same isolation/scrub contracts, subject "Your Scout login", spec copy) → **`/set-password` page** (login-styled; parses the verify-redirect fragment; friendly no-timeframe expired/used message; min 10 + confirm; `POST /auth/set-password` = bearer-verified `getUser` → `updateUserById`; writes `scout_session_v1` from the fragment tokens; relative redirect to `/dashboard` where the Connect Fathom card is the first action). Console: on `invite_sent` the temp-password box is HIDDEN ("Invite sent"); on failed/not_configured it shows exactly as before (fallback preserved, single create flow, presentation branch only). **Resend**: owner-only `POST /admin/users/:user_id/welcome-email` + per-row console button (fresh link supersedes old). Canonical host everywhere = `https://www.scoutsystems.io` (`CANONICAL_ORIGIN` in config.js; apex HTTPS times out). **Justin-side prerequisites: Supabase Auth URL Configuration — Site URL → https://www.scoutsystems.io, Additional Redirect URLs += /set-password (until then mints fail loudly by design), OTP expiry → 24h.** No token/credential in any log; page scrubs the URL fragment from history on load. Original temp-password email v1 never shipped to a real user.** `lib/welcome-email.js`: Google Workspace SMTP via nodemailer (smtp.gmail.com:465), sent as WELCOME_SMTP_USER (justin@scoutsystems.io). Env: WELCOME_SMTP_USER / WELCOME_SMTP_PASS / WELCOME_FROM_NAME on Railway — missing/blank = feature silently OFF (creation unchanged, console notes it). sendWelcomeEmail NEVER throws (KB/digest isolation); the hook in POST /admin/users runs AFTER the rollback-able section so no email outcome can unwind creation; response gains `welcome_email: sent|failed|not_configured` and the console shows it plainly with manual copy as the permanent fallback. Temp password appears in the email body ONLY — never logged, and error reasons are scrubbed (≥12-char token-ish runs masked) in case upstream echoes content. Transport = lazy singleton verified ONCE at boot (`welcomeEmail.init()` in index.js). Subject "Your Scout login", plain-text body per Justin's exact spec. Tests: welcome-email.test.js (content-exact, env matrix, isolation, scrub).
  2. Stage 5 account page, including billing sections.
  3. Onboarding + landing page "front door" pass. **Must include (logged 2026-07-23): forced password change on first login + a self-serve forgot/reset flow.** Today NOTHING forces a change and no reset UI exists anywhere — welcome emails therefore carry a PERMANENT credential (the §5 recon gap). The welcome email already says "please change your password after your first login"; this item makes that sentence true.
  4. Slack connect for EOD auto-post.
  5. Zoom / Google Meet as recording sources.
