...
- GitHub Releases: v1.0.0 initial, v1.0.1 macOS icon, v1.0.2 packaged-login fix (arm64), v1.0.3 attempted (three packaging bugs — never published), v1.0.4 (no shipped API keys, JWT refresh, proxy routing), v1.0.5 (cloud logging via SessionLogger), **v1.0.6 current release** — auto-updater 30-min polling + backend transient-failure retry + error truncation. Release published via `gh release create`.
