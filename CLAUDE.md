...
- **NEXT (not started — stopped for the checkpoint gate):** Stage 3 manager view (`/team/*`, `teamRepIds` driven by has-reps, `computeTeamAnalytics`, trends, team synthesis `type ∈ {team,highlights,digest}`, CRON digest); Stage 4 admin console rebuild (+ `managed_by`/`billing_status` PATCH, invite); Stage 5 frontend (Team view, console, account page). Full design + permissions matrix in the approved design doc (this session).
