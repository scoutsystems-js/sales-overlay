...
- GitHub Releases: v1.0.0 initial, v1.0.1 macOS icon, v1.0.2 packaged-login fix (arm64), v1.0.3 attempted (three packaging bugs — never published), **v1.0.4 current release** (arm64 + x64, no shipped API keys, JWT refresh, proxy routing)
...
## Future Plans
- **v1.0.5 priority: cloud logging / diagnostics dashboard.** Packaged app currently discards all console.log output (no terminal attached when launched from /Applications). Design: one shared `session_logs` table in Supabase (NOT per-user — that scales badly), with RLS restricting reads to the owning user. Client tees console.log/warn/error into a batched buffer, flushes every 2s or 20 lines to `/log` backend endpoint (which bulk-inserts via service role). Admin web view at `scoutsystems.io/admin` (allowlisted to justin@netrevenue.io only) renders a scrollable log view per session with a "Copy all" button for pasting to Claude during bug reports. Also creates `call_sessions` rows so Phase 3 analytics has real data from day one. Matches BUILD-PLAN.md Phase 3 "Diagnostics Dashboard" spec — pulling forward because debugging without it is guessing.
