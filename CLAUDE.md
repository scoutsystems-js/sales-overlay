...
- GitHub Releases: v1.0.0 initial, v1.0.1 macOS icon, v1.0.2 packaged-login fix (arm64), v1.0.3 attempted (three packaging bugs — never published), v1.0.4 (no shipped API keys, JWT refresh, proxy routing), **v1.0.5 current release** — cloud logging (SessionLogger tees console → Supabase via /log endpoints). Release published via `gh release create`.
...
## Future Plans
- **v1.0.6 TODO: add 30-min periodic auto-updater poll.** Current `initAutoUpdater` in `src/main/index.js:867` only calls `checkForUpdates()` ONCE, 5 seconds after launch. Long-running apps (e.g. partner's Scout left open across a release) never re-check and miss updates until they quit + relaunch. Fix: add `setInterval(checkForUpdates, 30 * 60 * 1000)` alongside the existing setTimeout. 30 min = 2 requests/hr/user, well under GitHub's 60/hr unauthenticated limit. Verification plan: ship v1.0.6 with the polling, install, keep running, then publish a v1.0.7 no-op bump; within 30 min the running v1.0.6 should log `[updater] Update available: 1.0.7` and pop the install dialog.
