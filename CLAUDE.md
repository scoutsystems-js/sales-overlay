...
- **BUILD PLAN (ordered, ratified 2026-07-22 close-out):**
  1. **Welcome emails on manual user creation — BUILT 2026-07-23, LOCAL COMMITS, awaiting approval to push.** `lib/welcome-email.js`: Google Workspace SMTP via nodemailer (smtp.gmail.com:465), sent as WELCOME_SMTP_USER (justin@scoutsystems.io). Env: WELCOME_SMTP_USER / WELCOME_SMTP_PASS / WELCOME_FROM_NAME on Railway — missing/blank = feature silently OFF (creation unchanged, console notes it). sendWelcomeEmail NEVER throws (KB/digest isolation); the hook in POST /admin/users runs AFTER the rollback-able section so no email outcome can unwind creation; response gains `welcome_email: sent|failed|not_configured` and the console shows it plainly with manual copy as the permanent fallback. Temp password appears in the email body ONLY — never logged, and error reasons are scrubbed (≥12-char token-ish runs masked) in case upstream echoes content. Transport = lazy singleton verified ONCE at boot (`welcomeEmail.init()` in index.js). Subject "Your Scout login", plain-text body per Justin's exact spec. Tests: welcome-email.test.js (content-exact, env matrix, isolation, scrub).
