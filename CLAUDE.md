...
- **Session start (mandatory):** Every new Claude Code session must cd into the project root before doing anything else: `cd '/Users/justinschmidt/Library/Mobile Documents/com~apple~CloudDocs/sales-overlay'`. This ensures MCP tools (including Supabase) are available for the session. Without this, Supabase MCP tools are not accessible and migrations must be run manually.
