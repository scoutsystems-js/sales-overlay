...
- **Routes served by backend:**
  ...
  - `/dashboard` → user dashboard (`backend/web/dashboard.html`; personal call history with inline log expansion; `/dashboard` auto-redirects admins/owners to `/admin`; per-user API at `backend/routes/me.js`)
  ...
  - `/me/*` → caller-scoped endpoints (`/me/sessions`, `/me/sessions/:id/logs` — `backend/routes/me.js`)
