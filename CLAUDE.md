...
- GitHub Releases: v1.0.0 initial, v1.0.1 macOS icon, v1.0.2 packaged-login fix (arm64), v1.0.3 attempted (three packaging bugs — never published), **v1.0.4 current release published 2026-04-23** (https://github.com/scoutsystems-js/sales-overlay/releases/tag/v1.0.4 — arm64 + x64, no shipped API keys, JWT refresh, proxy routing). Release published via `gh release create` (GitHub CLI now installed locally — no more manual web UI uploads for future releases).
...
## Future Plans
- **v1.0.5 IN PROGRESS: cloud logging / diagnostics dashboard.** Packaged app currently discards all console.log output (no terminal attached when launched from /Applications). Design: two shared tables in Supabase — `call_sessions` (one row per Start→Stop cycle, includes outcome + error_count + client_version) and `session_logs` (one row per log line, indexed by session_id and user_id+logged_at). RLS restricts reads to the owning user; INSERT happens via backend service role only. 14-day retention via `prune_old_session_data()` function scheduled by pg_cron in the Supabase dashboard. Migration at `backend/migrations/002_session_logs.sql`. Client tees console.log/warn/error into a batched buffer, flushes every 2s or 20 lines to `/log` backend endpoint (which bulk-inserts). Admin web view at `scoutsystems.io/admin` (allowlisted by email) renders a searchable session list → scrollable log view with "Copy all" for pasting into Claude during bug reports. Phases: (1) schema DONE, (2) backend endpoints DONE and live on Railway — `POST /log/session-start` creates a call_sessions row and returns session_id, `PATCH /log/session-end/:id` marks ended_at + optional outcome, `POST /log` validates + bulk-inserts batched entries (structural validation is a pure function `validateLogBatch` with 11 passing unit tests, covers null body, missing fields, max 100-entry batch, level defaulting, 4000-char message truncation, 32-char tag truncation). All three are auth+subscription gated. Existing routes (/health, /auth/*, /proxy/*, /download) regression-tested post-deploy — all green. Integration test against Supabase still pending — see commits `83847f7` + prior. (3) desktop SessionLogger class, (4) admin view, (5) build + release as v1.0.5 DMG. Matches BUILD-PLAN.md Phase 3 "Diagnostics Dashboard" spec.
