...
- **Stage 3 (Manager view) — SHIPPED, STOPPED for visual review 2026-07-21.** `/team/*` (manager+owner), rep set resolved+permission-checked server-side; `lib/team-analytics.js` (per-rep cards + team totals + trend arrows + week/month/quarter buckets) + `lib/team-synthesis.js` (team recommendations + Call Highlights of the Week — 3 lanes, cache types `team`/`highlights`, credit-tolerant, evidence resolved from real rows w/ rep attribution). Frontend: **Team** nav (manager+owner; Admin link now **owner-only**), `#team` route, owner team-picker, per-rep cards → pivot drill-through. Digest panel = pending (cron is the next stage). Verified live end-to-end. **DEMO TEAM (cleanable):** 3 reps `demo-{ava,ben,cara}@scout-demo.dev` assigned to Josh, with 15 of Josh's analyzed calls **copied** (new ids — non-destructive). To remove after review: delete those users + their copied `fathom_calls`/`call_analyses`/`call_highlights` (all `demo-`-prefixed `fathom_call_id`).
- **NEXT (not started — stopped for Justin's manager-view review):** Stage 4 admin console rebuild; Stage 5 (`/team/*`, `teamRepIds` driven by has-reps, `computeTeamAnalytics`, trends, team synthesis `type ∈ {team,highlights,digest}`, CRON digest); Stage 4 admin console rebuild (+ `managed_by`/`billing_status` PATCH, invite); Stage 5 frontend (Team view, console, account page). Full design + permissions matrix in the approved design doc (this session).
