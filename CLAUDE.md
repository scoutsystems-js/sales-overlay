...
- **Live Sessions toggle auto-hides** when the current (or pivoted `?user=`) target has zero `call_sessions` in the selected range (source: `computeAnalytics` `totals.total_calls`, already scoped to the pivoted user — no new query). If a user is on Live Sessions and changes to a zero-session range, `renderOverview` falls back to Coaching so they aren't stranded on a hidden tab.
