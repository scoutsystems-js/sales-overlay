...
**Observatory — approved 2026-09-12, implemented and live 2026-09-13:** Justin selected Observatory for Team → Coaching and Team → Performance. [site-design.md](site-design.md) records the approved appearance and byte-exact archives; [the implementation record](docs/design/observatory-implementation.md) records the verified live refinement commit `9bdfa4e1cbb8322a8f36add576650bf3be6323c5`. The September 13 refinement uses two localized emerald pools separated by a dark center, a fixed viewport background and HUD, and 24px desktop / 18px mobile gutters. The September 13 HUD restoration is live and verified in `3cbab93ffa9047103f3b0c66e96dfbf770053f89`: the fixed `1200×860` design has two arcs, two rings, four traces, three points, and a 28s scan; its opacity is `1`, pale strokes are `.105` / `.095`, its orbit is 82s, and its points are 67s / 91s / 74s. Background-off hides it and reduced motion stops it; the approved archives remain unchanged. These two views use scoped tokens, opaque panels, glow instruments, and a forest gradient in place of mesh. Their approved appearance supersedes the older visual rules below only within these two views. Other pages keep their existing design. Metric definitions, intelligence, behavior, and permissions are unchanged. Polished and Stark are historical; Atelier remains unapproved.
