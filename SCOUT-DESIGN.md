...
**Observatory — approved 2026-09-12:** Team → Coaching and Team → Performance are live; personal Coaching Dashboard (`overview`) is the approved third-page September 13 implementation; release verification: `~/Desktop/scan-reports/observatory-2026-09-13/overview/release-status.md`. [site-design.md](site-design.md) preserves the archives; [the implementation record](docs/design/observatory-implementation.md) records live receipts and the September 13 implementation. All three scoped views use the fixed two-pool forest ground, HUD, 24px desktop / 18px mobile gutters, opaque panels, and selected-nav treatment; existing metrics, behavior, permissions, and loading remain unchanged. HUD calibration is stroke `.14`, dash `.12`, points `.20`, scan `.17` at `.45`; archives preserve `.105`/`.095`. The direct Overview loader stamps its `data-view` hook before inserting the fixed HUD; a first-load fixture begins without that hook and pins it. The compact Overview refinement uses four existing-data instruments, a rail-height Coach Summary, a full-width lower focus grid, and a bounded two-by-two phone grid; release verification: `~/Desktop/scan-reports/observatory-2026-09-13/compact/release-status.md`. Full backend coverage: 2,856/2,856. Polished and Stark are historical; Atelier remains unapproved.
