...
**Observatory — approved 2026-09-12:** Team → Coaching and Team → Performance are live; personal Coaching Dashboard (`overview`) is the approved third-page September 13 implementation; release verification: `~/Desktop/scan-reports/observatory-2026-09-13/overview/release-status.md`. [site-design.md](site-design.md) preserves the archives; [the implementation record](docs/design/observatory-implementation.md) records live receipts and the September 13 implementation. All three scoped views use the fixed two-pool forest ground, HUD, 24px desktop / 18px mobile gutters, and selected-nav treatment; Team panels remain opaque, while personal Overview has the approved dark translucent green-tinted glass exception with modest blur and restrained sage/emerald edges. HUD calibration is stroke `.14`, dash `.12`, points `.20`, scan `.17` at `.45`; archives preserve `.105`/`.095`. The direct Overview loader stamps its `data-view` hook before inserting the fixed HUD; a first-load fixture begins without that hook and pins it. Overview uses three free-floating existing-data gauges: Closing %, objection handling %, and Avg call score. Closing links to Calls; focus begins below the hero beside the rail and Coach Summary ends the page. Release verification: `~/Desktop/scan-reports/observatory-2026-09-13/three-gauges/release-status.md`. Full backend coverage: 2,858/2,858. Polished and Stark are historical; Atelier remains unapproved.
...
| **Safety is layout, not faintness** — text sits on an opaque ground (`--bg`), never on dimmed artwork; the ground goes on before the cards come off; every per-view list must name a new view. Personal Overview is the approved scoped exception: dark translucent tint, modest blur, and rendered tint/edge guards preserve readable text over its fixed ground/HUD. | CLAUDE.md §4e (H286, H292, H637, H559); `dashboard.html` Overview glass block | `team-background.test.js` (the three carded containers), `page-treatment-pages.test.js`, `call-review-treatment.test.js`, `background-motifs.test.js`, `overview-observatory-rendered.test.js` |
