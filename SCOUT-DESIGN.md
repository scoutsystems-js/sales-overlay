...
| **Dashboard actions use** `.btn-fathom-primary` (accent fill, dark text). **Landing-page primary actions use the approved dark physical Visor control** (small green status light, raised dark surface); secondary landing actions are dark outlined controls. This landing exception is deliberate and must not be copied onto metric/action rows. | `dashboard.html` `.btn-fathom-primary` (~2052); `index.html` `.primary-visor` | **no guard** — a third treatment can ship unnoticed (finding) |
