...
// Manager daily digest — CACHE READ ONLY. Generation is CRON (next build stage);
// until then this returns available:false so the panel shows a pending state.
router.get('/digest', teamGate, async function (req, res) {
  try {
    var admin = getAdmin();
    var team = await resolveTeam(admin, req);
    var date = (req.query.date && /^\d{4}-\d{2}-\d{2}$/.test(req.query.date)) ? req.query.date : new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    if (!q.error && q.data && q.data.synthesis) return res.json(Object.assign({ available: true, date: date }, q.data.synthesis));
    res.json({ available: false, date: date, reason: 'The daily digest is generated by the overnight run (digest cron ships in the next stage).' });
