 ...
 */
/* ⚠ WINDOWS ARE PERCENTAGES, NOT PIXELS. A px offset that covers at one
   viewport width leaves a bare band at another, because the rendered size
   follows the viewport; 0-100% always covers. The px form is what left the
   account view 95% empty. */
function windowFor(i) {
  const gx = (i * 7) % 5, gy = (i * 3) % 4;
  return { xPct: Math.round(gx * 100 / 4), yPct: Math.round(gy * 100 / 3),
           x: Math.round((gx / 4) * (FW - W)), y: Math.round((gy / 3) * (FH - H)) };
}
...
function build({ seed, n, link, minSep }) {
  ...
  for (let tries = 0; pts.length < n && tries < n * 120; tries++) {
    ...
    /* ⚠⚠ DEPTH CARRIES BRIGHTNESS ONLY — DENSITY IS EVEN (Justin, 2026-08-20).
       Density used to follow depth, which is precisely what left bare patches:
       "reads as having depth" and "covers the whole background" were in direct
       conflict while ONE lever drove both. Separating them satisfies both —
       tone varies across the field, spacing does not.
       ⚠ Brightness-only depth still has to be SPATIAL. Per-node random depth
       AVERAGES TO FLAT at any real density (established two passes ago), so the
       smooth centres stay; they simply no longer thin the layout. */
...
/* ⚠⚠ 5x SPACING (Justin, 2026-08-20). minSep 52 -> 260 and link 120 -> 600 —
   BOTH scale by the same factor or nothing connects: at 5x spacing a 120-unit
   radius reaches no neighbours and the field becomes loose dots.
   ⚠ STROKES AND RADII ARE UNCHANGED, IN ABSOLUTE UNITS. Scaling the field by a
   transform would have thickened everything with it, which is the opposite of
   the ask. Spacing is a coordinate change; weight is not.
   ⚠ THE FIELD DOES **NOT** GROW. background-size is 200%, so the field renders
   at twice the viewport whatever its intrinsic size — growing FW/FH would just
   scale back down and cancel the spacing. Node COUNT falls instead, which is
   what "stretched out" means: fewer dots, further apart, same weight. */
const FIELD_PARAMS = { seed: 20260820, n: Math.round(340 * (FW * FH) / (W * H)), link: 300, minSep: 130 };
...
if (process.argv.includes('--write')) {
  ...
  const block = '    :root { --motif-mesh: url("' + FIELD.uri + '"); }\n'
    + out.map(r => `    body[data-view="${r.view}"]::before { background-position: ${r.win.xPct}% ${r.win.yPct}%; }`).join('\n');
