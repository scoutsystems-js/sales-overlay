...
function storedExamples(record,analysis,materialHash,meta){if(materialHash&&typeof materialHash==='object')materialHash=/^call-period-review-v[1-4]$/.test(record?.version)?materialHash.legacy:materialHash.current;const context=prepare(analysis)||contextFromSlice(record);if(!context||![VERSION,'call-period-review-v10','call-period-review-v9','call-period-review-v8','call-period-review-v7','call-period-review-v6','call-period-review-v5','call-period-review-v4','call-period-review-v1','call-period-review-v2','call-period-review-v3'].includes(record?.version)||record.source_hash!==context.hash||record.kb_hash!==materialHash||!Array.isArray(record.findings))return null;
 ...
 /* Block 002 (SCOUT-BUILD-SESSION.md): the located purchase decision travels with the example as `decision_turns` — the cited
    turns that ARE the decision the two fact reads agreed on. It is the one cross-stage signal a stored record can establish
    without a new model pass; rep-period-coaching reads it. Re-located from the stored reads here, for every version. */
 const S=require('./period-stage-facts');const reads=record.stage_facts?.reads;const facts=Array.isArray(reads)&&reads.length===2?S.finish(context,...reads):null;
 const decisionTurns=new Set(facts&&facts.status==='checked'?facts.decisions.flatMap(d=>d.turn_ids||[]):[]);
 ...
 if(record.version===VERSION&&(!facts||!S.allows(f,facts,context)))return null;
 ...
 result.push({...f,...meta,outcome:analysis.outcome,decision_turns:f.turn_ids.filter(n=>decisionTurns.has(n)),decision_located:decisionTurns.size>0,evidence:f.turn_ids.map(n=>({speaker:context.turns[n-1].speaker,quote:context.turns[n-1].text,timestamp_seconds:context.turns[n-1].time}))});}return result;}
