...
function buildSectionGraderPrompt(normalized, durationSeconds, sellingContext, coachingAreas, opts) {
  ...
  var lines = [
    ...
    // the work was absent.
    'STAGE RULES — decide whether each stage became due on THIS recording before you score anything, and grade only the work that was due and can be judged.',
    ...
    '  - Discovery is not a required script. Information the prospect volunteers, information established later in the call, and an accurate paraphrase all count; credit equivalent information and later answers, and do not demand a redundant question. A failure to get an answer is not a failure to ask. A later objection or an open or lost outcome can reveal a discovery gap but never proves one.',
    ...
    '  - Objection Handling is scored only when, after the pitch and the price (or a proven earlier presentation), the prospect resisted the purchase itself. A question is not resistance. A concern raised before the price is a disclosure that belongs to the stage where it happened, never an objection. A prospect who has decided to buy and is solving a mechanical problem with you — a payment-plan question, a card limit, a card that will not run, funds that clear later — is not objecting; that is logistical, and if nothing else was resisted the stage is not_applicable. A prospect who genuinely cannot afford it is disqualified, not objecting: do not grade the closer for failing to overcome genuine inability.',
    '  - Financing and buy-now-pay-later can be valid exceptions to a savings or credit guideline: falling below a guideline alone is not a disqualification, an unfinished or errored financing application is unresolved rather than ruled out, and correctly exploring a payment path is not a fault. Never invent financing availability or approval.',
    ...
    '{',
    '  "stage_assessment": {"context":{"sales_conversation":true|false|null,"call_kind":"initial"|"follow_up"|"unknown","ending":{"state":"completed"|"appropriate_continuation"|"cut_off"|"unknown","evidence_turn_ids":[1]},"pitch":{"occurred":true|false|null,"evidence_turn_ids":[1]},"price":{"occurred":true|false|null,"evidence_turn_ids":[1]},"prior_presentation":{"established":true|false|null,"evidence_turn_ids":[1]},"objection":{"occurred":true|false|null,"evidence_turn_ids":[1]},"finance":{"state":"qualified"|"genuine_dq"|"unresolved"|"not_assessed","discovered_stage":"discovery"|"late"|null,"feasible_financing_ruled_out":true|false|null,"evidence_turn_ids":[1]},"close_due":true|false|null},"stages":[{"stage":"intro"|"discovery"|"pitch"|"objection_handling"|"close","state":"evaluated"|"expected_but_missed"|"not_applicable"|"unmeasured","score":N|null,"grade":"A+"|"A"|"B"|"C"|"D"|"F"|null,"evidence_turn_ids":[1],"reason":"short factual explanation"|null}]},',
