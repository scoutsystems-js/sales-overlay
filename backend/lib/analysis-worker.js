...
const ANALYSIS_PROMPT_VERSION = 'v53-2026-09-05'; // v53: sentence-by-sentence evidence review. v52: one compact coaching brief, 45–65 word target. v51: scope/length preflight and original review IDs. v50: observed continuation without invented causation.
...
async function coachCallMoments(admin, fathomCallId, outcome, later, objectionNotes, userId) {
  ...
  var prompt = coachingLib.buildCoachingPrompt(moments, { evidenceContext: contexts.map(function(c,i){return 'COMPLETE EXCHANGE FOR MOMENT '+(i+1)+' (bounded excerpt plus call ending):\n'+evidenceReview.block(c);}).join('\n\n'), outcome: outcome, later: later, dq: callHasDq, historyBlock: evidenceReview.memoryBlock(verifiedHistory),
