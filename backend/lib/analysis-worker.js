...
function buildHighlightExtractorPrompt(normalized) {
  ...
  return [
    ...
    '  - closer_response: the closer\'s actual words responding to it — a contiguous verbatim span copied exactly per the HOW TO QUOTE rules above. INCLUDE EVERY WORD BETWEEN THE START AND END OF YOUR SPAN, including short interjections and filler you would naturally leave out ("You know what I mean?", "Right.", "So..."). Dropping a few words from the middle breaks the span and the quote is discarded, so a shorter unbroken span always beats a longer tidied one. Use null if the closer never responded, or if you cannot produce an exact span.',
  ...
  try {
    ...
    if (coachingAreas.length) {
      console.log('[analysis] what_mattered call=%s area=%s inverted=%s (closer_spoken=%d prospect_spoken=%d)',
        fathomCallId, whatMattered ? whatMattered.area_key : 'none',
