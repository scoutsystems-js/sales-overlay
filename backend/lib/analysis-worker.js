...
function coverageInstruction(areas) {
  ...
  return [
    ...
    '  - barrier_trace: if a concrete OBSTACLE surfaced late in this call — a financing shortfall, a declined amount, a third party who must approve, a hard timing constraint — and one of the areas you marked covered:false is ground that WOULD PLAUSIBLY HAVE SURFACED IT had it been established earlier, return {"area_key":"...","obstacle_quote":"..."}. obstacle_quote must be THE PROSPECT\'S OWN line stating the obstacle, copied EXACTLY as a contiguous run of words from ONE transcript line. ⚠ RETURN null FREELY. An uncovered area being AVAILABLE is not the same as it being the CAUSE — do not link an obstacle to a gap merely because the gap is the only one, or the nearest one. Only link where establishing that ground earlier would genuinely have surfaced this obstacle before it bit. If no obstacle surfaced, or no uncovered area plausibly explains it, return null.',
...
async function analyzeCall(fathomCallId, userId) {
  ...
  try {
    ...
    var rawTrace = graderParsed.barrier_trace;
    var barrierTrace = (roleInv.inverted || !rawTrace || typeof rawTrace !== 'object')
      ? null
      : resolveWhatMattered(
          { area_key: rawTrace.area_key, reason_evidence: rawTrace.obstacle_quote },
          { coverage: coverageOut, areas: coachingAreas,
            turns: normalized.turns, speakerConfidence: normalized.speaker_confidence });
    if (barrierTrace) {
      barrierTrace = { area_key: barrierTrace.area_key, obstacle_quote: barrierTrace.reason_evidence, reason_verified: true };
    }
