...
async function coachCallMoments(admin, fathomCallId, outcome, later, objectionNotes, userId) {
  ...
  try { history = (await coachHistory.loadHistory(admin, [userId]))[userId] || {}; }
  ...
  var coachedBefore = coachable.map(function (h) { return Object.assign({ pattern_key: coachHistory.patternKey(h) }, h); });
  var prompt = coachingLib.buildCoachingPrompt(moments, { evidenceContext: contexts.map(function(c,i){return 'COMPLETE EXCHANGE FOR MOMENT '+(i+1)+' (bounded excerpt plus call ending):\n'+evidenceReview.block(c);}).join('\n\n'), outcome: outcome, later: later, dq: callHasDq, historyBlock: coachHistory.historyBlock(history, coachedBefore),
  ...
  var reviewReply = await createWithUsage({model:coachingLib.CLAUDE_COACHING_MODEL,max_tokens:evidenceReview.MAX_REVIEW_TOKENS,messages:[{role:'user',content:evidenceReview.buildReviewPrompt(parsed,moments,contexts,material,outcome)}]}, {userId:userId||null,callId:fathomCallId,lane:'coaching-review'});
  var reviewParsed = extractFirstJsonObject(stripCodeFences(reviewReply.content.map(function(c){return c.text||'';}).join('')));
  parsed = evidenceReview.approvedEntries(parsed,reviewParsed,contexts,material);
  var written = 0;
  for (var k = 0; k < parsed.length; k++) {
    ...
    var patch = { coaching: entry.coaching.trim(), timestamp_seconds:contexts[idx].anchor, coaching_review:{version:evidenceReview.VERSION,verdict:'approved',kb_hash:material.kbHash,context_hash:contexts[idx].hash,anchor:contexts[idx].anchor} };
