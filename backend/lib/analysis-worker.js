...
const VALID_HIGHLIGHT_TYPES = [
  ...
  'rapport_moment',
  'disqualify_signal',
  'prospect_left',   // H725 (v43): the prospect ends, defers or withdraws WITHOUT a stated reason the offer does not apply
];
...
const ANALYSIS_PROMPT_VERSION = 'v61-2026-09-10'; // v61 (H768): a genuine financial DQ makes Objection Handling and Close not_applicable whenever discovered — one miss, coached once, in Discovery; enforced in the validator (stage-eligibility-v18), stated here so the reasons agree with the states.
...
const HIGHLIGHT_MAX_TOK  = 4500;             /* 8 highlights × ~300 tokens each, PLUS (v41) a cause on every buying signal (~170 tokens: two or three
                                                 evidence lines + a summary) and a disclosure tier on every prospect moment (~80). Measured before the
                                                 bump (H719): 174 logged extractor runs, p50 828 / p90 1,177 output tokens; the six longest calls by turn
                                                 count (2,601–3,134 turns) sit at ~1,000–1,430 by their highlight text; worst case with the new fields
                                                 ≈ 2,600 — under 3,000 but with no margin, and 2 of 174 runs ALREADY hit 3,000 (a transcript echo,
                                                 unparseable: stop_reason=max_tokens). 4,500 = the grader's cap; a derived limit is where the thing
                                                 breaks, not where it should sit. */
...
function extractFirstJsonArray(text) {
  ...
  // A transcript reference in leading prose is not the requested JSON array.
  // Prefer the explicitly fenced response before scanning arbitrary brackets.
  var fenced = text.match(/```(?:json)?[ \t]*\r?\n([\s\S]*?)```/i);
  if (fenced) {
    var fencedArray = parseJsonTolerant(fenced[1]);
    if (Array.isArray(fencedArray)) return fencedArray;
  }
  var start = cleaned.indexOf('[');
  if (start === -1) return null;
  var depth = 0;
...
// Production stage columns come from the parsed normal-grader response. A bad
// stage payload withholds stage metrics only; normal call analysis still saves.
function stageColumnsFromGrader(graderParsed, turns) {
  try {
    var assessment = stageEligibility.assessProduction(graderParsed, turns);
    return stageEligibility.toProductionColumns(
      assessment,
      formatSeconds,
      turns
    );
  } catch (err) {
    var detail = (err && err.message) ? err.message : 'unknown normal-grader stage failure';
...
function formatTurnsForPrompt(turns) {
  return turns.map(function(t, index) {
    var ts = (t.start_seconds != null) ? formatSeconds(t.start_seconds) : '--:--:--';
    return '[turn ' + (index + 1) + ' | ' + ts + '] ' + t.speaker + ': ' + (t.text || '');
  }).join('\n');
}
...
function buildSectionGraderPrompt(normalized, durationSeconds, sellingContext, coachingAreas, opts) {
  ...
  var lines = [
    'You are an expert sales coach reviewing a transcript of a high-ticket sales call. Determine which of the five sales stages occurred or became due, then grade only the work that can be judged from this recording.',
    '',
    ...
    '',
    'SCORING SCALE — whole-number score and letter MUST match exactly:',
    '  98-100: A+ — exceptional',
    '  90-97 : A — excellent',
    '  80-89 : B — strong',
    '  70-79 : C — adequate with real gaps',
    '  60-69 : D — weak',
    '  0-59  : F — significant failure or barely attempted work that was genuinely due',
    'Do not grade against a perfect-call ideal. Do not lower a score for correctly omitted work, and do not assign a score simply because a call ended without a sale.',
    '',
    ...
    '  1. INTRO       — did the closer establish useful direction for the conversation that actually took place: frame and control, credibility without overselling, an agenda the prospect agreed to. The opening is judged when the conversation actually starts, allowing greetings and technical trouble (there is no first-60-seconds rule); a meeting that correctly cannot proceed is judged on the direction actually established, not on unused script lines or a full agenda it never needed.',
    ...
    '  3. PITCH       — solution framing tied directly to discovery findings. A concern or doubt the prospect raises during the pitch, before the price, and the closer answers well in the pitch is pitch work and raises this score; it is not an objection and does not make Objection Handling due.',
    ...
    '',
    'STAGE ASSESSMENT — this is the sole source for stored stage scores. Return exactly five records: intro, discovery, pitch, objection_handling, close. For each record return state, whole-number score/grade only when gradeable, the strongest transcript turn identifiers within the bounds below, and a short plain-language reason. Never copy or reconstruct a quote: cite the turn identifiers and code will attach the actual speaker, timestamp, and text.',
    ...
    '  - not_applicable: this stage legitimately did not become due. score and grade are null, never zero.',
    '  - unmeasured: the recording/source cannot reliably establish whether the work was due or how it was performed. score and grade are null, never zero.',
    'A correct financial DQ found during Discovery can end the progression: grade Discovery, then set later stages not_applicable. A financial DQ found only after attempting to close can support a Discovery expected_but_missed score when the missed qualification is evidenced; do not call inability to buy a Close failure. Objection Handling may be gradeable only for actual resistance after pitch and price (or a proven prior presentation). No objection means not_applicable. A correctly booked continuation is not a failed purchase Close. A cut-off recording is not proof of a missed later stage. A follow-up grades only work due in that follow-up. Opening scheduling or availability alone is not Intro or Discovery work.',
    // v58 (Justin, 2026-09-07): the stage rules the offline candidate and reviewer prompts carried and
    // the production grader never saw. Written so that relabelling cannot satisfy them: work that was
    // not due is not_applicable, and a score asserts the work was done — it is never a way to say
    // the work was absent.
    // v59 (Justin, 2026-09-08): the doctrine's stage-relevant entries, read from the FILE at build time
    // through the one doctrineBlock — never a pasted copy. The hand copy of v58 is gone wherever the
    // file supplies the rule; what remains below has no equivalent in the doctrine.
    stageEligibility.graderDoctrineBlock(),
    'STAGE RULES — decide whether each stage became due on THIS recording before you score anything, and grade only the work that was due and can be judged. These sit beside the method above and do not repeat it.',
    '  - A stage that never became due on this recording is not_applicable. It is never a low score and never expected_but_missed. A score is a claim that the work was done and done at that level; a score is never a claim that work was absent. If you are about to score a stage for what did not happen, stop: either the stage did not become due (not_applicable) or the recording cannot tell you whether it became due (unmeasured).',
    '  - expected_but_missed requires a located exchange in this transcript showing the work was due before the call could legitimately pause or end, and that the closer then skipped it. A rebook, a real interruption, a recording that cuts off, or spare minutes before it are not that exchange. Available time and general usefulness never make work due.',
    '  - A correctly paused call is judged on the useful work actually done before the pause, not on what a full call would have contained. When a legitimate blocker ends the conversation — a required decision maker is absent, the prospect is at work or cannot talk now, a mechanical problem stops payment — and a specific continuation is agreed, the stages that were not reached are not_applicable, and the remaining discovery and qualification may be completed on the resumed call. A factual absence during a legitimate pause is not itself a fault.',
    '  - Establishing that a required decision maker is absent IS decision-maker qualification, even in the opening: asking whether the partner will join and rebooking when they cannot is evaluated Discovery, not not_applicable, and a correct pause does not make the other discovery areas due.',
    '  - Discovery is not a required script. Information the prospect volunteers, information established later in the call, and an accurate paraphrase all count. A failure to get an answer is not a failure to ask. Never require extra work merely because it might be useful.',
    '  - On a follow-up, the stages correctly completed on the earlier call are not_applicable on this one; grade only the work that became due in this conversation. Do not make the rep repeat completed stages.',
    '  - A cut-off recording cannot manufacture a missed stage and cannot lower the score of a stage that was underway: judge what was recorded, mark the rest unmeasured or not_applicable, and never grade a stage on what had not yet happened when the recording stopped.',
    '  - Objection Handling is scored only when the method above finds an objection — resistance to the purchase itself, after the pitch and the price (or a proven earlier presentation). A concern raised before the price is a disclosure that belongs to the stage where it happened, and handling it well there is credited there. A logistical barrier or a genuine inability to pay is not resistance; if nothing else was resisted, the stage is not_applicable.',
    // v60 (Justin, 2026-09-08): the partner objection is POSITIONAL, not tonal. A model reading for
    // resistance will miss the most common objection in this business every time, because a partner
    // objection is agreeable by nature.
    '  - THE PARTNER OBJECTION IS DECIDED BY POSITION, NEVER BY TONE. After the pitch and the price (or a proven earlier presentation), a prospect who defers the decision to a spouse, partner, parent, business partner or any other person whose agreement they say they need has raised a partner objection. It is an objection because of WHEN it was said, not how it was said: it stays one no matter how agreeable, polite, willing or friendly the prospect sounds. "I\'ll talk to my wife and get back to you" is what a partner objection sounds like every time; "the prospect expressed no resistance" is never a reason to set this stage not_applicable when such a deferral followed the price. Set objection.occurred true with that turn and score Objection Handling on what the closer did with it. Before the pitch and the price, the same sentence is a decision-maker disclosure and belongs to Discovery — that placement is unchanged.',
    '  - An unfinished or errored financing application is unresolved rather than ruled out, and correctly exploring a payment path is not a fault.',
    // v61 (Justin, 2026-09-10; H768): a DQ is one miss, coached once. The validator holds this on the finance context; the sentence is here so the reasons agree with the states.
    '  - A GENUINE FINANCIAL DISQUALIFICATION — the prospect cannot fund the purchase — makes Objection Handling and Close not_applicable WHENEVER it is discovered: a disqualification is not an objection and is not a failed close. Discovery carries the miss once. Found early, Discovery scores the qualification work and pitch, objection and close are not due; found late, Discovery is marked down for the qualification that was missed and Objection Handling and Close are still not_applicable. A prospect who can pay and does not want to is an objection, never a disqualification; an unresolved or unassessed financing question changes nothing here.',
    '  - Close is the purchase decision, not the end of the recording and not the booking of another meeting. Score it only when a purchase decision was due. A correctly booked continuation is good follow-up behaviour and earns no purchase-Close grade; booking a next call does not excuse avoiding a decision that WAS due.',
    // v59 (Justin, 2026-09-08): the circular close. A rep cannot decide whether a rep gets graded.
    '  - A prospect asking what it costs makes a purchase decision due from that moment, unless a blocker located in this transcript prevented it — a required decision maker absent, a prospect who has to leave, a recording that ends. A real blocker is a thing that happened. The closer\'s own reason for not giving the price is not a blocker, and the closer\'s reason for not asking for the decision is not one either: when the price was asked for and the closer declined it, set close_due true and judge the Close on what the closer did with that moment. A price question asked in passing during discovery, where the conversation legitimately continued toward the pitch and the prospect was not left waiting for an answer, does not by itself make a Close due; and where a real blocker with an agreed continuation ended the call, the continuation rule above stands and the Close is not_applicable.',
    '  - A brief description of the offer or an answer to a question is not automatically a pitch; a pitch frames the solution against what discovery found. A sales script is not proof an opportunity occurred, and team material cannot turn a pre-price concern into an objection or make every question mandatory during an appropriate pause.',
    'The stage reason is optional display prose, not evidence. Keep it short and factual. If no safe reason is available, return null. Never let prose replace the cited source turns.',
    'For broader coaching context only, return an optional `objection` notes object below. It is not a stage score; `stage_assessment` is the only stage metric source.',
    '',
    ...
    '  - sales_call_verdict: exactly one of "sales" | "not_sales" | "unsure". "sales" = a real prospect is on the call being sold to at ANY stage — a first call, a follow-up, a two-minute reconnect, a logistics-only chat with someone already sold all count. "not_sales" = nobody is being sold to: internal staff only, a training, a role-play, a coaching debrief, a review of someone else\'s recorded call (even though it contains a pitch), a test recording, an empty room. "unsure" = you genuinely cannot tell from the transcript. Never guess: when the transcript could be read either way, say "unsure". ⚠ A prospect who is only NAMED, expected, or spoken ABOUT but never speaks in the recording is NOT on the call: a recording of the closer alone trying to reconnect, waiting, or narrating is "not_sales" with class "recording_stub" — the verdict must agree with your own reason: if your reason says no sales conversation took place, the verdict cannot be "sales". ⚠ A real person on the call who is NOT a buyer — a regulator, a vendor, a partner, an auditor, someone doing due diligence, a colleague — is not a prospect: nobody is being sold to, so the verdict is "not_sales" (class "no_prospect_internal_staff" for staff, otherwise the class that fits) — again, the verdict must agree with your own reason: if your reason says the person was not there to buy, the verdict cannot be "sales".',
    ...
    '{',
    '  "stage_assessment": {"context":{"sales_conversation":true|false|null,"call_kind":"initial"|"follow_up"|"unknown","ending":{"state":"completed"|"appropriate_continuation"|"cut_off"|"unknown","evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"pitch":{"occurred":true|false|null,"evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"price":{"occurred":true|false|null,"evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"prior_presentation":{"established":true|false|null,"evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"objection":{"occurred":true|false|null,"evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"finance":{"state":"qualified"|"genuine_dq"|"unresolved"|"not_assessed","discovered_stage":"discovery"|"late"|null,"feasible_financing_ruled_out":true|false|null,"evidence_turn_ids":'+stageEligibility.CONTEXT_EVIDENCE_SCHEMA+'},"close_due":true|false|null},"stages":[{"stage":"intro"|"discovery"|"pitch"|"objection_handling"|"close","state":"evaluated"|"expected_but_missed"|"not_applicable"|"unmeasured","score":N|null,"grade":"A+"|"A"|"B"|"C"|"D"|"F"|null,"evidence_turn_ids":'+stageEligibility.STAGE_EVIDENCE_SCHEMA+',"reason":"short factual explanation"|null}]},',
    '  "objection": {"notes":"brief context for broader coaching"} | null,',
    '  "overall_score": N,',
    '  "overall_summary": "...",',
    '  "one_thing": "...",',
  ...
  if (sellingContext && sellingContext.trim()) {
    var at = lines.indexOf('SCORING SCALE — whole-number score and letter MUST match exactly:');
    if (at !== -1) {
      lines.splice(at, 0,
...
function buildHighlightExtractorPrompt(normalized) {
  ...
  return [
    ...
    /* H725 (Justin, 2026-09-04): six of eight failed missed-signal pairs ended on the prospect LEAVING typed as a
       disqualification. A reschedule, a hang-up, a withdrawal is not a reason the offer does not apply. */
    '      "prospect_left"      — the prospect ENDS, DEFERS or WITHDRAWS from the call — asks to reschedule or "get back to you", says they have to go, hangs up or shuts the conversation down after a confrontation, pulls out — WITHOUT a stated reason the offer does not apply. THE DISCRIMINATOR: a stated REASON the offer does not apply is a disqualify_signal; LEAVING WITHOUT ONE is prospect_left. Never type a leaving as a disqualification.',
    '',
    ...
    '',
    arcCause.causePromptBlock(),   // H719 — the ONE cause block, shared with the backward pass
    '',
...
/* H725: a leaving (`prospect_left`) is the prospect's and never a closer line, so the anchor guard
   refuses it too — but it is NOT a type a closer response bears on (no handling badge), so it stays
   OUT of PROSPECT_POSITION_TYPES, which the review page mirrors as HANDLING_TYPES (③-5). */
var LEAVING_TYPE = 'prospect_left';
...
function violatesProspectAnchor(h) {
  ...
  if (PROSPECT_POSITION_TYPES.indexOf(h.type) === -1 && h.type !== LEAVING_TYPE) return false;
...
function sanitizeHighlights(arr, durationSeconds) {
  ...
  for (var i = 0; i < arr.length && out.length < MAX_HIGHLIGHTS; i++) {
    ...
    out.push({
      ...
      /* H719: shape-only here; attachArcFields VERIFIES every line against the transcript before
         persist. A cause belongs to a buying signal, a disclosure tier to a prospect-spoken
         moment; anything else the model offers is dropped, and nothing is invented (null =
...
/* H719 — verify the arc fields against the transcript. Returns a NEW array so a
   caller that calls this and drops the result keeps NOTHING (the H666 second
   plant): the highlights it returns carry the verified shape, the input is
   untouched. On an unmatched transcript every label is a guess, so every
   claimed move and tier is refused (`none — not_evidenced`) — a closer-side
   lane requires proven closer lines, never inferred ones. */
function attachArcFields(highlights, turns, speakerConfidence) {
  ...
  var out = (Array.isArray(highlights) ? highlights : []).map(function (h) {
    ...
    if (c.cause) {
      c.cause = arcCause.verifyCause(c.cause, turns, c.timestamp_seconds, locate);
      if (c.cause && c.cause.refused) stats.refused++; else if (c.cause && c.cause.move === 'none') stats.none++; else if (c.cause) stats.moves++;
    }
  ...
  console.log('[analysis] arc fields moves=%d none=%d refused=%d tiers=%d tiers_refused=%d', stats.moves, stats.none, stats.refused, stats.tiers, stats.tiers_refused);
  return out;
}

/* H721 — the selectivity bar at the one write point. Applied AFTER attachArcFields (the
   buying-signal leg reads the VERIFIED cause) and BEFORE persist. Returns a NEW array. It
   is a filter on what is CAPTURED, never a delete: persistHighlights' no-wipe rule means an
   empty survivor set preserves the call's existing rows. Loud, so the live per-call number
   can be read against the 4.2 prediction. */
function applyMomentBarToCall(highlights, fathomCallId) {
  var r = momentBar.applyMomentBar(highlights);
  console.log('[analysis] moment bar call=%s kept=%d dropped=%d (%s)', fathomCallId, r.kept.length, r.dropped.length,
    r.dropped.map(function (h) { return h.type; }).join(',') || 'nothing');
  return r.kept;
}
...
async function analyzeCall(fathomCallId, userId) {
  ...
  try {
    ...
    /* H731: one_thing is an INSTRUCTION to the closer. With no team material behind the grader it is advice made up on
       the fly — withheld. The scores and the section notes (the reasoning for a number) stay. */
    if (graderParsed && !(selling.contextText && selling.contextText.trim())) { graderParsed.one_thing = null; graderParsed.one_thing_timestamp_seconds = null; console.log('[analysis] one_thing withheld: no team material (call=%s)', fathomCallId); }
    if (!graderParsed) {
    ...
    // compatibility column: those come only from its validated stage record.
    ...
    // Stage assessment is part of the already-required normal grader response.
    // No candidate, independent-review, or factual-proof model request belongs
    // in the production call path; those facilities remain offline QA tools.
    var stageColumns = stageColumnsFromGrader(graderParsed, normalized.turns);
    ...
    // H719: the cause and the disclosure tier are proven against the transcript or refused in public.
    sanitizedHighlights = attachArcFields(sanitizedHighlights, normalized.turns, normalized.speaker_confidence);
    // H721: the bar. What is captured, never what is kept from before.
    sanitizedHighlights = applyMomentBarToCall(sanitizedHighlights, fathomCallId);
...
async function coachCallMoments(admin, fathomCallId, outcome, later, objectionNotes, userId) {
  var res = await admin.from('call_highlights')
    .select('id, type, resolution, section, timestamp_seconds, quote, observation, speaker, closer_response, closer_response_verified, objection_category, objection_class, handling')   // H735: the pattern key and the DQ class read these
    .eq('fathom_call_id', fathomCallId);
  ...
  /* ⚠⚠ H731 — THE KNOWLEDGE BASE, BEFORE THE ADVICE. One retrieval (lib/kb-material): the team's offer,
     qualifications, script, uploads and coaching notes, scoped by the relationship rules and inherited from
     the head. NOTHING RELEVANT → THIS LANE SAYS NOTHING: no model call, no coaching written, the skip named. */
  var material;
  try { material = await require('./kb-material').loadKbMaterial(admin, { userId: userId, lane: 'coaching', maxChars: 2500 }); }
  catch (kmErr) { console.warn('[coaching] call=%s knowledge base unavailable (%s) \u2014 saying nothing', fathomCallId, (kmErr && kmErr.message) || 'unknown'); return { selected: moments.length, written: 0, skipped: 'kb_unavailable' }; }
  if (!material.hasMaterial) return { selected: moments.length, written: 0, skipped: 'no_material' };
  // A new pass invalidates its old advice first. Failure must not leave a stale
  // approval behind. This is call-scoped, never a historical backfill.
  for (var old of (res.data || [])) {
    var cleared = await admin.from('call_highlights').update({coaching:null,coaching_review:null}).eq('id',old.id).eq('fathom_call_id',fathomCallId);
    if (cleared.error) return {selected:0,written:0,skipped:'review_reset_failed'};
  }

  var evidenceReview = require('./coaching-evidence-review');
  var analysisRead = await admin.from('call_analyses').select('transcript_stored,outcome,why_outcome').eq('fathom_call_id', fathomCallId).maybeSingle();
  if (analysisRead.error || !analysisRead.data) return {selected:moments.length,written:0,skipped:'evidence_unavailable'};
  var located = coachable.map(function (h) { return {highlight:h,context:evidenceReview.contextFor(h, analysisRead.data)}; }).filter(function (x) { return x.context; });
  if (!located.length) return {selected:moments.length,written:0,skipped:'evidence_unlocated'};
  coachable = located.map(function (x) { return x.highlight; });
  var contexts = located.map(function (x) { return x.context; });
  if (JSON.stringify(contexts).length > 120000) return {selected:coachable.length,written:0,skipped:'evidence_too_large'};
  moments = coachable.map(function (h,i) { return coachingLib.toMoment(Object.assign({},h,{timestamp_seconds:contexts[i].anchor})); });
  var corrections = material.notes || { rows: [], text: '', hash: 'none' };
  var callHasDq = coachingLib.hasDqMoment(res.data || []) || outcome === 'disqualified';   // H733: never a lost deal
  /* H735 — THE RECORD, read before the prompt: what Scout has coached this closer on before (by pattern), so a repeat
     is coached at its cause. A read failure coaches without history and says so. */
  var coachHistory = require('./coaching-history');
  var history = {}; var callDateForHistory = null;
  try { var cdq = await admin.from('fathom_calls').select('call_date').eq('id', fathomCallId).maybeSingle(); callDateForHistory = (cdq && cdq.data && cdq.data.call_date) || null; } catch (cdErr) { callDateForHistory = null; }
  try { if (callDateForHistory && Number.isFinite(Date.parse(callDateForHistory))) history = (await coachHistory.loadHistory(admin, [userId], null, new Date(Date.parse(callDateForHistory) - 1).toISOString()))[userId] || {}; }
  catch (hErr) { console.warn('[coaching] call=%s coaching record unavailable (%s) \u2014 coaching without history', fathomCallId, (hErr && hErr.message) || 'unknown'); }
  var verifiedHistory = evidenceReview.historyFacts(history, coachable, fathomCallId);
  var prompt = coachingLib.buildCoachingPrompt(moments, { actionRecords: require('./coaching-action-record').buildRecords(contexts), outcome: outcome, later: later, dq: callHasDq, historyBlock: evidenceReview.memoryBlock(verifiedHistory),
    objectionNotes: objectionNotes || null, managerNotes: corrections.text || null, sellingContext: material.contextText || null,
    doctrineBlock: material.doctrineBlock ? material.doctrineBlock('coaching') : '' });   // H732
  ...
  var parsed = extractFirstJsonArray(raw);
  if (!Array.isArray(parsed)) return { selected: moments.length, written: 0, skipped: 'unparseable' };
  // Explicit ids only: after filtering, positional fallback can attach advice to another moment.
  parsed = parsed.filter(function(e,i,all){return e && Number.isInteger(e.moment) && e.moment>=1 && e.moment<=moments.length && evidenceReview.safeAdvice(e.coaching) && all.filter(function(x){return x && x.moment===e.moment;}).length===1;});
  parsed = coachingLib.enforceHardRules(parsed, { hasDq: callHasDq });   // H732: the two hard rules, in code, on the written text

  if (!parsed.length) return {selected:moments.length,written:0,skipped:'no_supported_draft'};
  // Deterministic failures do not need another model request. Keep original IDs
  // after filtering, and retain failures below so the withheld reason is stored.
  var reviewable = parsed.filter(function(e){return !evidenceReview.draftProblem(e,contexts[e.moment-1],verifiedHistory[e.moment-1]);});
  var reviewParsed = null;
  if (reviewable.length) {
    var reviewReply = await createWithUsage({model:coachingLib.CLAUDE_COACHING_MODEL,max_tokens:evidenceReview.MAX_REVIEW_TOKENS,messages:[{role:'user',content:evidenceReview.buildReviewPrompt(reviewable,moments,contexts,material,outcome,verifiedHistory)}]}, {userId:userId||null,callId:fathomCallId,lane:'coaching-review'});
    reviewParsed = extractFirstJsonObject(stripCodeFences(reviewReply.content.map(function(c){return c.text||'';}).join('')));
  }
  var decisions = evidenceReview.evaluateEntries(parsed,reviewParsed,contexts,material,verifiedHistory);
  for (var decision of decisions) {
    if (decision.verdict === 'approved') continue;
    var refused = moments[decision.moment - 1];
    var refusalWrite = await admin.from('call_highlights').update({coaching:null,coaching_review:{version:evidenceReview.VERSION,verdict:'withheld',category:decision.category,reason:decision.reason,kb_hash:material.kbHash,context_hash:contexts[decision.moment-1].hash}}).eq('id',refused.id).eq('fathom_call_id',fathomCallId);
    if (refusalWrite.error) return {selected:moments.length,written:0,skipped:'review_status_write_failed'};
  }
  parsed = evidenceReview.approvedEntries(parsed,reviewParsed,contexts,material,verifiedHistory);
  var written = 0;
  for (var k = 0; k < parsed.length; k++) {
    var entry = parsed[k];
    if (!entry || typeof entry.coaching !== 'string' || !entry.coaching.trim()) continue;
    // Explicit ids were validated before review; never attach by array position.
    var idx = entry.moment - 1;
    var m = moments[idx];
    if (!m) continue;
    ...
    var patch = { coaching: entry.coaching.trim(), timestamp_seconds:contexts[idx].anchor, coaching_review:{version:evidenceReview.VERSION,verdict:'approved',category:'approved',knowledge_refs:decisions.find(d=>d.moment===entry.moment).knowledge_refs,history_ref:verifiedHistory[idx] && entry.coaching.includes(verifiedHistory[idx].text) ? verifiedHistory[idx].id : null,kb_hash:material.kbHash,context_hash:contexts[idx].hash,anchor:contexts[idx].anchor} };
    ...
    /* H735: a repeat is never an accusation — the scold shapes are dropped in code before the write */
    if (coachHistory.scoldsRepeat(patch.coaching)) { console.warn('[coaching] dropped (scolds a repeat): ' + patch.coaching.slice(0, 120)); continue; }
    var up = await admin.from('call_highlights').update(patch).eq('id', m.id);
    if (!up.error) written++;
    /* H735: THE RECORD IS WRITTEN with the entry — rep × pattern × call, so the next time is the (N+1)th, not the first */
    if (!up.error) { var src = coachable[idx] || {}; try { await coachHistory.recordCoaching(admin, { user_id: userId, team_key: (material && material.teamKey) || null, moment: src, fathom_call_id: fathomCallId, highlight_id: m.id, call_date: callDateForHistory || null, version: ANALYSIS_PROMPT_VERSION }); } catch (rErr) { console.warn('[coaching-history] ' + ((rErr && rErr.message) || 'unknown')); } }
  }
...
module.exports = {
  ...
  DOCTRINE_FILE_SHA1: DOCTRINE_FILE_SHA1,
  ...
  _stageColumnsFromGrader:     stageColumnsFromGrader,
