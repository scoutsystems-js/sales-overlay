...
const ANALYSIS_PROMPT_VERSION = 'v49-2026-09-05'; // v49: source IDs, verified memory and classified review outcomes. v48: complete coaching exchanges, independent evidence/KB review, code-owned timestamps. v47 (H735): the rep's coaching record reaches the prompt — a repeat is coached at its cause, never as an accusation. Was v46-2026-09-05 // v46 (H733): the manager's notes ride under the doctrine entries they speak to; a disqualified prospect is never a lost deal in the coaching prompt. Was v45-2026-09-05
...
async function coachCallMoments(admin, fathomCallId, outcome, later, objectionNotes, userId) {
  ...
  var reviewReply = await createWithUsage({model:coachingLib.CLAUDE_COACHING_MODEL,max_tokens:evidenceReview.MAX_REVIEW_TOKENS,messages:[{role:'user',content:evidenceReview.buildReviewPrompt(parsed,moments,contexts,material,outcome,verifiedHistory)}]}, {userId:userId||null,callId:fathomCallId,lane:'coaching-review'});
  var reviewParsed = extractFirstJsonObject(stripCodeFences(reviewReply.content.map(function(c){return c.text||'';}).join('')));
