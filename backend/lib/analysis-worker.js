...
function coverageInstruction(areas) {
  ...
  return [
    ...
    '  - missed_cue: did the PROSPECT raise a concern or context clue EARLY that the closer did NOT dig into — he acknowledged it, reframed it or moved on rather than qualifying on it — and did a concrete OBSTACLE surface LATER in the call (a financing shortfall, a declined amount, a third party who must approve, a hard timing constraint)? If so return {"cue_quote":"...","obstacle_quote":"..."}. BOTH must be THE PROSPECT\'S OWN lines, each copied EXACTLY as a contiguous run of words from ONE transcript line. ⚠ RETURN null FREELY. Proximity in time is NOT causation: an unaddressed remark and a later obstacle are often unrelated, and most calls have neither. Only return a pair where digging into that specific cue would plausibly have surfaced that specific obstacle before it bit. Do not reach for the nearest available pair.',
...
function buildHighlightExtractorPrompt(normalized) {
  ...
  return [
    ...
    '      "barrier"            — a concrete EXTERNAL constraint the prospect CANNOT decide their way out of. Apply the same test in reverse: if deciding differently would not change it, it is a BARRIER. A lender approving only $5,000 against a $9,800 price, a spouse who must sign, funds locked until a date — the prospect cannot simply choose otherwise. Something that must be SOLVED, not reframed.',
    ...
    '  - handling: exactly one of "addressed", "deflected", "ignored". THE TEST: did the closer engage with the SUBSTANCE of what the prospect raised, or acknowledge it and move on? Warmth, length and sympathy are NOT engagement — a long, kind reply that never touches the concern is "deflected". Worked example: after a prospect disclosed losing $300,000 on past ventures, the closer said "Don\'t bring the ex-girlfriend into the conversation with a date with the hot blonde. I respect it. I know what you went through." That is DEFLECTED — it validates the feeling and never touches the concern. Use "ignored" when the closer did not respond at all.',
...
async function analyzeCall(fathomCallId, userId) {
  ...
  try {
    ...
    // ─── 9a: a cue the closer did not dig into, and what came later ─────
    // Anchored on a MISSED CUE, not an abstract coverage gap. Coverage cannot
    // distinguish "never came up" from "came up and was ignored", and that
    // distinction is the entire point — the old area-anchored version linked on
    // 1 call in 14 and its one link was weak.
    //
    // resolveWhatMattered's area checks do not apply here (there is no area to
    // check), so what carries over is its PROVING primitive — labelForQuote —
    // applied to BOTH quotes, plus a deterministic separation check.
    var barrierTrace = null;
    var traceReject = null;
    var rawTrace = graderParsed.missed_cue;
    if (!roleInv.inverted && rawTrace && typeof rawTrace === 'object'
        && typeof rawTrace.cue_quote === 'string' && typeof rawTrace.obstacle_quote === 'string'
        && normalized.speaker_confidence === 'matched') {
      var cueLoc = locateQuoteSpeaker(normalized.turns, rawTrace.cue_quote);
      var obsLoc = locateQuoteSpeaker(normalized.turns, rawTrace.obstacle_quote);
      var cueRole = labelForQuote(normalized.turns, rawTrace.cue_quote);
      var obsRole = labelForQuote(normalized.turns, rawTrace.obstacle_quote);

      if (!cueLoc.ok || !obsLoc.ok) traceReject = 'quote_not_reconstructible';
      else if (cueRole !== 'PROSPECT' || obsRole !== 'PROSPECT') traceReject = 'not_prospect_words';
      else {
        var cueTs = num((normalized.turns[cueLoc.index] || {}).start_seconds);
        var obsTs = num((normalized.turns[obsLoc.index] || {}).start_seconds);
        if (cueTs === null || obsTs === null) traceReject = 'no_timestamps';
        else if (obsTs - cueTs < MIN_CUE_GAP_SECONDS) traceReject = 'too_close';
        else barrierTrace = {
          cue_quote: rawTrace.cue_quote, cue_timestamp_seconds: cueTs,
          obstacle_quote: rawTrace.obstacle_quote, obstacle_timestamp_seconds: obsTs,
          gap_seconds: obsTs - cueTs, verified: true,
        };
      }
      if (traceReject) console.log('[analysis] missed_cue rejected (%s) call=%s', traceReject, fathomCallId);
    }
