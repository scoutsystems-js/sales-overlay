...
function buildHighlightExtractorPrompt(normalized) {
  ...
  return [
    ...
    '      "objection"          — the PROSPECT\'S OWN RESISTANCE to going ahead, expressed AFTER the price has been stated AND the close has been asked for. A true objection only happens once a real decision is on the table. Two tests, both required: (a) is it after price and the ask, and (b) is it their own stance rather than an external fact? "I could probably get there, but not today" is an objection — it is their position. "The loan only approved $5,000" is a barrier — it is a fact. Concerns raised BEFORE price is on the table are NOT objections however money-shaped they sound; file those as risk_signal or barrier.',
    ...
    '      "barrier"            — a concrete EXTERNAL obstacle rather than an attitude, and rather than the prospect\'s own resistance: a financing shortfall or declined amount, a third party whose approval is needed, a fixed timing constraint. Something that must be SOLVED. A barrier is a FACT the prospect is REPORTING, not a position they are TAKING.',
