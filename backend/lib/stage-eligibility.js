...
function promptInstructions(sellingContext) {
 return [
  ...
  'Return one JSON object with context and sections. sections MUST be an array of exactly five entries: exactly one each for intro, discovery, pitch, objection, close. Do not omit any stage. If a stage cannot be safely scored, return its unmeasured or not_applicable state with null grade, null score, and empty note. Each entry has section naming the stage plus:{"assessment":{"state":"evaluated|not_applicable|expected_but_missed|unmeasured","evidence_turns":[]},"grade":"A|B|C|D|F"|null,"score":0-100|null,"notes":"one factual observation, max35words"|null}. A scored entry has no more than eight evidence_turns. Never copy source quotations or times; code attaches them from evidence_turns. Never cite internal turn numbers in notes. Notes may not contradict context or the stage assessment. Not-applicable notes are empty.',
...
function validProducerSections(parsed) {
 if (!Array.isArray(parsed?.sections)) return true; // Legacy internal callers pass keyed records; provider output is always the array form.
 return parsed.sections.length===SECTIONS.length&&parsed.sections.every(row=>SECTIONS.includes(row?.section))&&new Set(parsed.sections.map(row=>row.section)).size===SECTIONS.length;
}
