...
const stageEvidenceTurns=object(Object.fromEntries(Array.from({length:8},(_,index)=>['turn_'+(index+1),nullableInteger])));
const assessment=object({state:choice(['evaluated','not_applicable','expected_but_missed','unmeasured']),reason:choice(['observed_work','appropriate_continuation','early_financial_dq','late_financial_dq','not_reached','no_objection','recording_incomplete','not_sales','missing_work','insufficient_evidence']),evidence_turns:stageEvidenceTurns});
const stage=object({assessment,grade:{type:'string',description:'A+, A, B, C, D or F for a measured stage; empty otherwise.'},score:{type:['integer','null']},notes:{type:'string',description:'A scored note is one sentence of one or two factual clauses using only the grammar Closer asked...; Prospect said.... Empty otherwise.'}});
