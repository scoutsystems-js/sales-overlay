...
// Every string that only the manual Add-to-KB controls produce.
const GONE = [
  'sec-kb-btn',            // A: the drilldown button class
  'sec-saved',             // A: its saved badge
  'addSectionMomentToKb',  // A: its handler
  'review-kb-btn',         // B: the review-card button class
  'addMomentToKb',         // B: its handler
  'addToKb',               // B: the render opt that switched it on
  'loadSavedMoments',      // B: the saved-set fetch
  'savedMomentIds',        // B: the saved-set state
  'saved_to_kb',           // A: the server-marked saved flag it read
];

test('BOTH manual Add-to-KB controls are gone from the render path', () => {
...
test('⚠ NON-VACUITY — the matcher actually catches a reintroduced control', () => {
  ...
  const broken = LIVE + '\n<button class="review-kb-btn" onclick="addMomentToKb(1)">+ Add to KB</button>';
  const found = GONE.filter((s) => broken.indexOf(s) !== -1);
  assert.ok(found.indexOf('review-kb-btn') !== -1 && found.indexOf('addMomentToKb') !== -1,
    'the matcher must see a reintroduced button, or this proves nothing');
});
 ...
 */
test('the manager→team gap is real: harvest writes personal, personal is uploader-only', () => {
  const harvest = fs.readFileSync(path.join(__dirname, '..', 'lib', 'kb-harvest.js'), 'utf8');
