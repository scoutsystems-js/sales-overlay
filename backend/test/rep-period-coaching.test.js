...
test('uses earned Close eligibility scores and exposes exact ties rather than a fabricated separation',()=>{const result=P.summarize([{id:'a',call_date:'2026-09-01',analysis_status:'done',analysis:analysis({discovery_score:40,close_score:100,close_score_earned:40})}],[],{from:'2026-09-01',to:'2026-09-02'});assert.deepEqual(result.tied_sections,['discovery','close']);assert.equal(result.sections.find(s=>s.section==='close').score,40);});
