...
test('manager card shows scope, named evidence, outcome and owner-aware links at desktop/mobile widths',()=>{
 ...
 const fn=['ymd','dayLabel','rangeLabelInclusive','managerFollowupHtml'].map(n=>fnBody(live,n)).join('\n');
 for(const width of [1400,390]){
 const html='<html><head>'+source.slice(source.indexOf('<style>'),source.indexOf('</style>')+8)+'</head><body data-view="team-coaching"><main id="host" class="page team-coaching-workspace"></main><script>var MONTH_SHORT=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function escapeHtml(s){return String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/"/g,"&quot;");}function outcomeLabel(){return "Open";}function formatTimestampDisplay(){return "01:40";}function openCallReview(c,u){window.opened=[c,u];}'+fn+';document.querySelector("#host").innerHTML=managerFollowupHtml('+JSON.stringify(priority)+');</script></body></html>';
 ...
 assert.equal(r.overflow,false);assert.equal(r.scripts,0);assert.deepEqual(r.opened,['c1','rep']);assert.match(r.text,/Drill booking/);assert.match(r.text,/Sep 1 - Sep 5/);assert.match(r.text,/3 of 8 assessed/);assert.match(r.text,/Open/);assert.match(r.text,/Pat <script>/);assert.doesNotMatch(r.text,/Manager action|doctrine|knowledge base/);assert.equal(r.link,example.clip_url);
 }
});
