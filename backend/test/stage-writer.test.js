...
const A=require('../lib/stage-assessment');
...
candidate.discovery={assessment:{state:'evaluated',reason:'observed_work',evidence_turns:[1]},grade:'C',score:75,notes:'Closer asked who would decide.'};
candidate.pitch={assessment:{state:'evaluated',reason:'observed_work',evidence_turns:[3]},grade:'C',score:72,notes:'Closer stated the price.'};
candidate.close={assessment:{state:'evaluated',reason:'observed_work',evidence_turns:[4]},grade:'B',score:84,notes:'Prospect said they would buy today.'};
...
test('offline QA candidate/review lane persists only reviewed eligibility fields',async()=>{
 const calls=[];
 const material={contextText:'Offer',kbHash:'material'};
 const result=await A.runReviewedQa({turns,duration:30,material},async request=>{
  calls.push(request);return calls.length===1?candidate:review;
 });
 assert.equal(calls.length,2,'offline QA candidate/review does not run factual-proof reads');
 const fields=S.toReviewedColumns(result.record,String,turns,S.guidanceHash(material));
 ...
 assert.equal(persisted.discovery_grade,'C');
...
test('normal-grader stage failure writes five unmeasured stages without failing the normal payload',()=>{
 const fields=W._stageColumnsFromGrader({},turns);
