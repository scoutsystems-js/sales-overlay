...
test('⚠ the two prompt lanes whose category line changed order carry a NEW lane version (a prompt change is a cache bump, H442)', () => {
  ...
  assert.ok(/SYNTH_RULE_VERSION = 'v5-2026-09-02-category-order-canonical'/.test(perf));
  assert.ok(/RECS_LANE_VERSION = 'v8-2026-09-02-category-order-canonical'/.test(recs));
});
